  ...
  return (
    <header className="bg-white shadow-md fixed top-0 w-full z-50">
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        <Link
          to={user ? '/home' : '/'}
          className="flex items-center gap-2.5 text-green-700 text-2xl font-bold"
        >
        ...
        <nav className="hidden md:flex items-center gap-8 text-base font-medium">
          {!user && (
            <>
              <button onClick={() => handleAnchorClick('about')} className="hover:text-green-600">
                About Us
              </button>
              <button onClick={() => handleAnchorClick('impact')} className="hover:text-green-600">
                Impact
              </button>
              <button onClick={() => handleAnchorClick('manifesto')} className="hover:text-green-600">
                Manifesto
              </button>
              <button onClick={() => handleAnchorClick('faq')} className="hover:text-green-600">
                FAQ
              </button>
          ...
          {!loadingAuth && !user && !isAuthPage && (
            <>
              ...
              <Link
                to="/login"
                className="bg-white text-green-600 border border-green-600 px-4 py-2 rounded-md font-medium hover:bg-green-50"
              >
                Login
              </Link>
            </>
            ...
            {!loadingAuth && !user && !isAuthPage && (
              <div className="flex gap-3">
                <Link
                  to="/signup"
                  onClick={closeMenu}
                  className="flex-1 bg-white text-green-600 border border-green-600 px-4 py-2 text-center rounded-md"
                >
                  Sign Up
                </Link>
                <Link
                  to="/login"
                  onClick={closeMenu}
                  className="flex-1 bg-green-600 text-white px-4 py-2 text-center rounded-md"
                >
                  Login
                </Link>
              </div>
